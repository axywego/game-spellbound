...
std::vector<Tile*> Tilemap::getCollisionTilesInRange(const sf::Vector2f &pos, const float &range) const {
    ...
    int minGridX = static_cast<int>(std::floor(bounds.position.x / cellSize));
    int maxGridX = static_cast<int>(std::floor((bounds.position.x + bounds.size.x) / cellSize));
    int minGridY = static_cast<int>(std::floor(bounds.position.y / cellSize));
    int maxGridY = static_cast<int>(std::floor((bounds.position.y + bounds.size.y) / cellSize));

    // int gridWidth = static_cast<int>(spatialCollisionGrid.size());
    int gridWidth = static_cast<int>(worldSize.x);
    if (gridWidth == 0) return ret;

    // int gridHeight = static_cast<int>(spatialCollisionGrid[0].size());
    int gridHeight = static_cast<int>(worldSize.y);

    minGridX = std::max(0, minGridX);
    maxGridX = std::min(gridWidth - 1, maxGridX);
    minGridY = std::max(0, minGridY);
    maxGridY = std::min(gridHeight - 1, maxGridY);

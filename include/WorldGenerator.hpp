...
TiledShape createRectangle(const sf::Vector2i& pos, const sf::Vector2i& size){
    ...
    for(int i = 0; i < limitsTiles.y; i++){
        for(int j = 0; j < limitsTiles.x; j++){
            auto& el = shape[i][j];
            el.setPosition({j * tileSize, i * tileSize});
            el.setSize({tileSize, tileSize});
...
TiledShape generateRectangle(){
    ...
    while(posRectangle.x + sizeRectangle.x >= limitsTiles.x && 
     posRectangle.y + sizeRectangle.y >= limitsTiles.y){
        posRectangle = {generate8Bytes(0, limitsTiles.x * 3 / 4 ), generate8Bytes(0, limitsTiles.y * 3 / 4)};
        sizeRectangle = {generate8Bytes(limitsTiles.x / 4, limitsTiles.x * 3 / 4), generate8Bytes(limitsTiles.y / 4, limitsTiles.y * 3 / 4)};
    }
...
private:
    TiledShape part;
public:
    Room(): part(TiledShape(limitsTiles.y, std::vector<sf::RectangleShape>(limitsTiles.x))) {
        init();
    }
    ...
    void init(){
        for(size_t i = 0; i < limitsTiles.y; i++){
            for(size_t j = 0; j < limitsTiles.x; j++){
                auto& el = part[i][j];
                el.setFillColor(sf::Color::Black);
                el.setPosition({i * tileSize, j * tileSize});
                el.setSize({tileSize, tileSize});
            }
        }
    }
    ...
    void setPosition(const sf::Vector2i& newPos) {
        ...
        for(auto& row : part){
            for(auto& el : row){
                el.move({delta.x * tileSize, delta.y * tileSize});
            }
        }
    }
...
public:
    ...
    static TiledShape generateDungeon() {
        ...
        for(int i = 0; i < numRooms; i++){
            ...
            part.setCenterPosition({mapSize.x / 2 + radius * std::cosf(rad),
                 mapSize.y / 2 + radius * std::sinf(rad)});

            addRoom(part);

            addCorridor(mapSize / 2, part.getCenterPosition(), 2);

            rad += M_PI / 180.f * (360 / numRooms);
